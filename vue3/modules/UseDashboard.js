...
function useDashboard({ allTickets, users, channels }) {
  ...
  const CHANNEL_COLORS = { whatsapp: '#25D366', telegram: '#2AABEE', webchat: '#6C63FF' };
  const STATUS_COLOR   = { open: '#22c55e', pending: '#f59e0b', closed: '#64748b' };
  const STATUS_LABEL   = { open: 'Aberto', pending: 'Pendente', closed: 'Encerrado' };
  ...
  // ── Helpers de data ───────────────────────────────────────────────────────
  const _toDate     = val => { if (!val) return null; const n = Number(val); return !isNaN(n) ? new Date(n) : new Date(val); };
  const _hoje       = () => new Date().toISOString().split('T')[0];
  const _isHoje     = val => _toDate(val)?.toISOString().split('T')[0] === _hoje();
  const _horasAtras = val => (Date.now() - (_toDate(val)?.getTime() ?? 0)) / 3600000;

  // ── Separação grupo x atendimento ────────────────────────────────────────
  const dashServiceTickets = computed(() =>
    allTickets.value.filter(t => !t.isGroup)
  );
  ...
  const dashTicketsAbertos   = computed(() => dashServiceTickets.value.filter(t => t.status === 'open'));
  const dashTicketsPendentes = computed(() => dashServiceTickets.value.filter(t => t.status === 'pending'));

  // Abertos de hoje vs anteriores ainda em aberto
  const dashAbertosHoje      = computed(() => dashTicketsAbertos.value.filter(t => _isHoje(t.createdAt)));
  const dashAbertosAnteriores = computed(() => dashTicketsAbertos.value.filter(t => !_isHoje(t.createdAt)));

  // Fechados só de hoje (KPI de produtividade)
  const dashFechadosHoje = computed(() =>
    dashServiceTickets.value.filter(t => t.status === 'closed' && _isHoje(t.closedAt))
  );

  // Tickets em risco de SLA: abertos ou pendentes há mais de DASH_SLA_HORAS sem userId
  const dashTicketsSlaRisco = computed(() =>
    [...dashTicketsAbertos.value, ...dashTicketsPendentes.value].filter(t =>
  ...
  const dashStats = computed(() => ({
    ...
    // Fila total atual
    filaTotal:         dashTicketsAbertos.value.length + dashTicketsPendentes.value.length,
    // SLA
  ...
  const dashAvgTime = computed(() => {
    const finished = dashFechadosHoje.value.filter(t => t.closedAt && t.createdAt);
    if (!finished.length) return '—';
    const avg = finished.reduce((acc, t) =>
      acc + (_toDate(t.closedAt) - new Date(t.createdAt)), 0) / finished.length;
  ...
  const dashChannelStats = computed(() =>
    channels.value.map(ch => {
      ...
      return {
        ...
        fechadosHoje:     tks.filter(t => t.status === 'closed' && _isHoje(t.closedAt)).length,
      };
    })
  );
  ...
  const dashAgentStats = computed(() =>
    users.value.map(u => {
      ...
      return {
        ...
        fechadosHoje: tks.filter(t => t.status === 'closed' && _isHoje(t.closedAt)).length,
        total:        tks.filter(t => t.status === 'open').length +
                      tks.filter(t => t.status === 'closed' && _isHoje(t.closedAt)).length,
      };
    })
  );
  ...
  const dashPieData = computed(() => [
    { name: 'Abertos hoje',      value: dashStats.value.abertosHoje,       color: '#22c55e' },
    { name: 'Abertos anteriores',value: dashStats.value.abertosAnteriores, color: '#ef4444' },
    { name: 'Pendentes',         value: dashStats.value.pendentes,          color: '#f59e0b' },
    { name: 'Fechados hoje',     value: dashStats.value.fechadosHoje,       color: '#64748b' },
  ]);
  ...
  const dashHourlyData = computed(() => {
    const hours = Array.from({ length: 10 }, (_, i) => ({ hour: `${8 + i}h`, count: 0 }));
  ...
  const dashFilteredTickets = computed(() => {
    let tks = dashServiceTickets.value.filter(t =>
      t.status === 'open' ||
      t.status === 'pending' ||
  ...
  const dashGroupByChannel = computed(() =>
    channels.value
      .map(ch => ({
        ...ch,
        icon:  CHANNEL_ICONS[ch.type] ?? '📡',
        count: dashGroupTickets.value.filter(t => t.channelId === ch.id).length,
      }))
  ...
  const dashFmtTime = isoStr => {
    if (!isoStr) return '—';
    return _toDate(isoStr)?.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
  };

  const dashFmtHorasAtras = isoStr => {
    const h = _horasAtras(isoStr);
    if (h < 1) return `${Math.round(h * 60)}m atrás`;
    return `${Math.floor(h)}h${Math.round((h % 1) * 60).toString().padStart(2,'0')} atrás`;
  };

  // Helpers para template
  const dashChannelIcon  = id => CHANNEL_ICONS[dashGetChannel(id)?.type]  ?? '📡';
  const dashChannelColor = id => CHANNEL_COLORS[dashGetChannel(id)?.type] ?? '#aaa';
  const dashChannelName  = id => dashGetChannel(id)?.name ?? '—';
  const dashUserName     = id => id ? (dashGetUser(id)?.name ?? '—') : 'Sem atendente';
  ...
  function dashResizeChart() { _barChart?.resize(); }
  ...
  function dashDisposeChart() {
    ...
    if (_barChart) { _barChart.dispose(); _barChart = null; }
  }
