 ...
 */
function UseSettings({ URL_BASE, token }) {
  ...
  const activeDaysCount = computed(
    () => schedule.value.filter((d) => d.enabled).length,
  );
  const weekendCount = computed(
    () =>
      schedule.value.filter(
        (d) => d.enabled && (d.dayOfWeek === 0 || d.dayOfWeek === 6),
      ).length,
  );
  const avgHoursPerDay = computed(() => {
    ...
    const total = active.reduce(
      (sum, d) => sum + parseHours(d.startTime, d.endTime),
      0,
    );
  ...
  async function loadHours() {
    try {
      const qp = selectedQueueId.value
  ...
  function applyToAll() {
    ...
    const src = schedule.value.find(
      (d) => d.dayOfWeek === Number(copyFromDay.value),
    );
    if (!src) return;
